...
const uploadPatterns = computed(() => traffic.value.filter(row =>
  Number(row.input_bytes || 0) >= 20 * 1024 ** 3 && Number(row.upload_ratio || 0) >= 4
));
async function load() {
  error.value = "";
  try {
...
<template>
  <div class="aiops-page radius-page">
    ...
    <section class="radius-analysis-kpis">
      ...
      <article class="card"><TrendingUp /><div><small>高上行结构观察</small><strong>{{ uploadPatterns.length }}</strong><em>≥20GB 且上下载比 ≥4</em></div></article>
    ...
    <article v-else class="card aiops-table-card">
      <header class="radius-card-head"><div><h2>高上行流量异常观察</h2><p>仅展示“当前窗口上行 ≥ 20 GB 且上下载比 ≥ 4”的账号；完整全网流量排行、总量和趋势在 Accounting 页面。</p></div></header>
      <div class="table-scroll"><table class="data-table"><thead><tr><th>账号</th><th>窗口上行</th><th>窗口下行</th><th>窗口合计</th><th>上下载比</th><th>会话</th><th>判断</th></tr></thead><tbody><tr v-for="row in uploadPatterns" :key="String(row.username)"><td><RouterLink class="radius-account-link" :to="{ path: '/radius', query: { keyword: row.username } }">{{ row.username }}</RouterLink></td><td>{{ bytes(row.input_bytes) }}</td><td>{{ bytes(row.output_bytes) }}</td><td><strong>{{ bytes(row.total_bytes) }}</strong></td><td>{{ row.upload_ratio }}</td><td>{{ row.sessions }}</td><td><span class="radius-observe-tag">需要观察</span></td></tr><tr v-if="!uploadPatterns.length"><td colspan="7" class="radius-empty">当前窗口没有命中高上行观察阈值的账号</td></tr></tbody></table></div>
    </article>
  </div>
</template>
