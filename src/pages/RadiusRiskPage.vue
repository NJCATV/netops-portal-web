...
import { ArrowDownAZ, Clock3, RefreshCw, Search, ShieldAlert } from "lucide-vue-next";
...
async function load() {
  ...
  error.value = "";
  try {
    const response = isMulti.value ? await loadRadiusMultiMac(hours.value, minMacs.value) : await loadRadiusRejectRisk(hours.value, 500);
