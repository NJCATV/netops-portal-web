...
function applyData(data: Awaited<ReturnType<typeof loadRadiusAccounting>>) {
...
onMounted(() => {
  const cached = readApiSnapshot<Awaited<ReturnType<typeof loadRadiusAccounting>>>(`radius:accounting:${hours.value}`);
  if (cached) {
    applyData(cached);
    loading.value = false;
  }
...
<template>
  <div class="aiops-page radius-page" :class="{ loading }">
    ...
    <RadiusModuleTabs />
    <div v-if="loading" class="aiops-notice">正在读取 Accounting 汇总快照…</div>
    <div v-if="error" class="aiops-notice error">{{ error }}</div>
    <div class="radius-accounting-note"><DatabaseZap :size="17" /><div><strong>当前统计覆盖 {{ observedDuration }} · 平均总速率 {{ averageRate }}</strong><p>流量按同一账号、NAS、Acct-Session-Id 的相邻累计计数器做差；首次报文和计数器回退只建立新基线，不计入流量。当前窗口识别到 {{ Number(quality.rollback_records || 0).toLocaleString() }} 条回退记录，已从汇总和异常排行中排除。</p><p>实际数据：{{ coverage.first_event_time || "-" }} 至 {{ coverage.last_event_time || "-" }}。首尾不足 10 分钟的图表点为不完整时间桶。</p></div></div>
    ...
    <article class="card aiops-table-card radius-anomaly-card">
      ...
      <div class="table-scroll"><table class="data-table"><thead><tr><th>账号</th><th>窗口上行</th><th>窗口下行</th><th>窗口总量</th><th>平均速率</th><th>回退记录</th><th>会话</th><th>命中规则</th><th>最后活动</th></tr></thead><tbody><tr v-for="row in visibleAnomalies" :key="String(row.username)"><td><RouterLink class="radius-account-link" :to="{ path: '/radius', query: { keyword: row.username } }">{{ row.username }}</RouterLink></td><td>{{ bytes(row.input_bytes) }}</td><td>{{ bytes(row.output_bytes) }}</td><td><strong>{{ bytes(row.total_bytes) }}</strong></td><td>{{ Number(row.average_mbps || 0).toFixed(1) }} Mbit/s</td><td>{{ Number(row.rollback_records || 0).toLocaleString() }}</td><td>{{ row.sessions }}</td><td><span class="radius-anomaly-tags"><i v-for="reason in anomalyReason(row)" :key="reason">{{ reason }}</i></span></td><td>{{ row.last_seen }}</td></tr><tr v-if="!visibleAnomalies.length"><td colspan="9" class="radius-empty">当前窗口没有账号命中流量异常规则</td></tr></tbody></table></div>
