...
const filters = reactive({ hours: 168, module: "", result: "", event_kind: "", user_keyword: "", keyword: "" });
...
const eventKindLabels: Record<string, string> = { page_view: "页面访问", user_action: "主动操作", security: "安全审计" };
...
function eventKindName(value: unknown) { const key = String(value || ""); return eventKindLabels[key] || key || "-"; }
...
function actionName(value: unknown) {
  ...
  const labels: Record<string, string> = { login: "用户登录", change_password: "修改密码", page_view: "进入页面", search: "执行搜索", run_ai_analysis: "执行 AIOps 分析", ai_chat: "AI 问答", export_quality_excel: "导出 ONU 质差 Excel", import_boss_users: "导入 BOSS 用户", dashboard_visit: "进入统一驾驶舱" };
...
function reset() { filters.module = ""; filters.result = ""; filters.event_kind = ""; filters.user_keyword = ""; filters.keyword = ""; void load(); }
...
<template>
  <div class="system-audit-page" :class="{ loading }">
    <section class="page-heading system-audit-heading">
      <div><span>系统管理 / 超级管理员</span><h1>系统审计与使用分析</h1><p>仅统计页面访问、主动操作和安全事件；保留搜索内容，不记录密码、令牌或请求正文。</p></div>
    ...
    <section class="card system-audit-filter">
      ...
      <label><span>行为类型</span><select v-model="filters.event_kind"><option value="">全部行为</option><option value="page_view">页面访问</option><option value="user_action">主动操作</option><option value="security">安全审计</option></select></label>
    ...
    <section class="system-audit-kpis">
      <article><span><Activity :size="19" /></span><div><small>真实行为记录</small><strong>{{ number(data.overview.request_count) }}</strong><em>已排除初始化与自动刷新</em></div></article>
    ...
    <section class="system-audit-grid">
      ...
      <article class="card system-audit-module"><header><div><h2>模块使用分布</h2><p>按用户主动行为统计，帮助识别高频功能。</p></div></header><div v-if="data.modules.length" class="system-audit-module-list"><div v-for="item in data.modules" :key="item.module"><span>{{ moduleName(item.module) }}</span><b>{{ number(item.request_count) }}</b><small>{{ number(item.active_users) }} 位用户 · {{ number(item.failure_count) }} 异常</small><i><em :style="{ width: `${Math.max(4, Number(item.request_count || 0) / Math.max(1, Number(data.modules[0]?.request_count || 0)) * 100)}%` }"></em></i></div></div><div v-else class="system-audit-empty">暂无模块行为数据</div></article>
    </section>
    ...
    <section class="card system-audit-log"><header><div><h2>审计明细</h2><p>共 {{ number(data.total) }} 条；后台加载与自动刷新已排除。</p></div></header><div class="table-scroll"><table class="data-table"><thead><tr><th>时间</th><th>用户</th><th>行为类型</th><th>模块 / 功能</th><th>结果</th><th>行为与搜索内容</th><th>耗时</th><th>来源 IP</th></tr></thead><tbody><tr v-for="item in data.items" :key="item.id"><td>{{ time(item.occurred_at) }}</td><td><strong>{{ item.display_name || item.username || "匿名" }}</strong><small>{{ item.username || "-" }} · {{ item.role_code || "-" }}</small></td><td><strong>{{ eventKindName(item.event_kind) }}</strong><small>{{ item.trigger_source || "-" }}</small></td><td><strong>{{ moduleName(item.module) }}</strong><small>{{ actionName(item.action) }}</small></td><td><span :class="['system-audit-result', item.result]">{{ resultName(item.result) }}</span><small>HTTP {{ item.status_code }}</small></td><td><code>{{ item.method }} {{ item.request_path }}</code><small v-if="item.search_content" class="system-audit-search-content">搜索：{{ searchContent(item.search_content) }}</small></td><td>{{ item.duration_ms == null ? "-" : `${item.duration_ms} ms` }}</td><td>{{ item.client_ip || "-" }}</td></tr></tbody></table></div><div v-if="!loading && !data.items.length" class="system-audit-empty">当前筛选条件下暂无日志</div><footer><button class="btn btn-secondary" :disabled="page <= 1 || loading" @click="load(page - 1)">上一页</button><span>第 {{ page }} / {{ totalPages }} 页</span><button class="btn btn-secondary" :disabled="page >= totalPages || loading" @click="load(page + 1)">下一页</button></footer></section>
  </div>
</template>
